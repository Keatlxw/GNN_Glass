...
class GNNLayer(nn.Module):
    def __init__(self, node_embedding_len, edge_embedding_len, n_head,
                 ...
                 remember_func="residual", device=None,use_graph_level_attention=False):
        """
        Define a GNN layer.
        Args:
        ...
        self.device = device
        self.use_graph_level_attention = use_graph_level_attention
        if self.use_graph_level_attention:
            self.graph_level_attention = nn.Sequential(
                Linear(node_embedding_len, 1),  # 将节点特征投影到一个值
                nn.Tanh(),  # 可以考虑使用Tanh等激活函数
                nn.Softmax(dim=0)  # 确保注意力权重的和为1
            )
    ...
    def forward(self, node_features, edge_features, neighbor_indices,
        ...
        #print('neighbor_len',neighbor_len)

        #print('neighbor_indices',neighbor_indices[:10])

        #print('neighbor_masks',neighbor_masks[:10])
        ...
        packed_concat_features = pack_padded_sequence(
            concat_features, neighbor_lens.cpu(), batch_first=True, enforce_sorted=False)
        ...
        packed_concat_features = pack_padded_sequence(
            ...
            neighbor_lens.cpu(), batch_first=True, enforce_sorted=False)
        ...
        #heads_attention_matrix = torch.zeros((4,200,20,1))
        ...
        for attention_linear, value_linear, attention_bn in zip(
            ...
            heads_attention_weights.append(head_attention)


            #print('heads',heads)

            #print('head_attention',head_attention[0])

            #heads_attention_matrix[heads] = head_attention

            heads = heads + 1

            # change head_attention to variable length PackedSequence.
            packed_head_attentions = pack_padded_sequence(
                head_attention, neighbor_lens.cpu(),
        ...
        if self.use_graph_level_attention:
            ...
            '''head_attention_weights = self.attention_linears[0](packed_concat_features.data)
            head_attention_weights = self.attention_softmax(head_attention_weights)
            head_attention_weights, _ = pad_packed_sequence(PackedSequence(
        ...
        # 计算图表示：加权节点特征和
            graph_feature = torch.sum(node_features_updated * head_attention_weights.squeeze(-1), dim=1)'''


            # 计算每个节点的注意力权重
            attention_weights = self.graph_level_attention(node_features).squeeze()
            #print(attention_weights.shape)
            #print(node_features.shape)
            attention_weights=attention_weights.reshape((4000,1))
            # 计算图级别的特征表示：节点特征的加权和
            graph_feature = torch.sum(attention_weights * node_features, dim=0, keepdim=True)
            return graph_feature, node_features_updated, edge_features_updated, (h, c),heads_attention_weights
        else:
            return 0,node_features_updated, edge_features_updated, (h, c), heads_attention_weights
